...
describe('App', () => {
  ...
  describe('mapDispatchToProps', () => {
    ...
    it ('should call dispatch with the loadingCards action when it is called', () => {
      const mockDispatch = jest.fn();
      const actionToDispatch = loadingCards(true);

      const mappedProps = mapDispatchToProps(mockDispatch);
      mappedProps.loadingCards(true);

      expect(mockDispatch).toHaveBeenCalledWith(actionToDispatch);
    })

    it ('should call dispatch with the resetSearchCards action when it is called', () => {
      const mockDispatch = jest.fn();
      const actionToDispatch = resetSearchCards();

      const mappedProps = mapDispatchToProps(mockDispatch);
      mappedProps.resetSearchCards();
